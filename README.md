...
| `--proxy URL`                         | Tunnel the connection through an HTTP CONNECT proxy, e.g. `http://proxy:8080` or `http://user:pass@proxy:8080` (host targets only); OCSP/CRL/CA-Issuer/CT-log requests use it too. Without it, the environment proxy (`HTTPS_PROXY`, honouring `NO_PROXY`) is used, like curl. |
| `--no-proxy`                          | Force direct connections (OCSP/CRL requests included), ignoring any proxy set in the environment. Mutually exclusive with `--proxy`.                                                                                                                                           |
...
auth is supported via `user:pass@`. The OCSP, CRL, CA-Issuer and CT-log
(`--discover`) requests use the same proxy, so revocation still works where
only the proxy reaches the Internet. Their host names are still resolved
locally for the internal-address check.
...
proxy on macOS/Windows) and honours `NO_PROXY`. Use `--no-proxy` to force
direct connections, for the OCSP/CRL requests too, regardless of the
environment.
