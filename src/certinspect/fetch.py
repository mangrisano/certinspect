...
def verify_chain(
    ...
    as a browser would. Return ``(True, None)`` if the chain and hostname are
    trusted, or ``(False, reason)`` if verification fails. Network-level
    failures are left to propagate.
    """
    context = ssl.create_default_context()
    try:
        with socket.create_connection((host, port), timeout=timeout) as sock:
...
def check_revocation(
    cert: x509.Certificate, timeout: float = 5.0
) -> tuple[str, str | None]:
